...
class CartFragment:Fragment() {
    ...
    private val animslist = ArrayList<AnimCart>()
        ...
        thread {
            try {
                ...
                val request = Request.Builder()
                    .url("${trueIp}:8080/homework/bilibili/cartoon.jsp")
                    .build()
                //执行
    ...
    // Anims动画类
    inner class AnimsAdapter(private val animslist:List<AnimCart>): RecyclerView.Adapter<AnimsAdapter.AnimsViewHolder>(){
    ...
    private fun refresh(jsonData:String){
        try {
            ...
            for(i in 0 until jsonArray.length()){
                ...
                val pinYing=jsonObject.getString("pinYing")
                data.add(AnimCart(id,name,description,image,pinYing))
                animslist.add(AnimCart(id,name,description,image,pinYing))
            }
            animslist.sortBy { user -> user.pinYing } 			// 指定以name属性进行升序排序
            //animslist.sortByDescending { user -> user.pinYing } // 指定以name属性进行降序排序
        }catch (e: Exception){
            e.printStackTrace()
        }
